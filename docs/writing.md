...
To confirm this still works after a dependency upgrade, add a throwaway post with
`draft: true`, run `npm run build`, and check its URL 404s in `dist/`. The sibling
GitHub Pages blog keeps a permanent `draft-not-for-production.md` for exactly this
check — worth copying the idea here if regression-proofing this becomes a habit.
