...
The published site will be static HTML on whatever host is eventually chosen — not yet
decided, so the rest of this section (inherited from the Cloudflare-hosted template this
repo was copied from) is illustrative rather than current. In general there are two
independent ways to change what visitors see: a commit to this GitHub repository (which
a dashboard Git integration or CI workflow would pick up and rebuild automatically), or a
change made directly in the host's own dashboard — build settings, environment
variables, custom domains, rollbacks — none of which touches git at all. So the security
of the blog is the security of *both* — the GitHub account/tokens that can write to the
repo, and whoever has access to the host's dashboard once one exists.
...
Cloudflare itself sits in front of every request as the CDN/edge host — that is
infrastructure, not a third-party embed, but worth knowing: Cloudflare can see and log
raw traffic to this site the way any host can.
...
on the [contact page](https://creativedigitalgrowth.bolt.host/contact/)
